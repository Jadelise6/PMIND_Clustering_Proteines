...
# BLAST
BLAST_COMP_FILE = f"{global_path}/reduced_blast_components.tsv"                  # Fichier prévu pour contenir les composantes de BLAST
PRIN_COMP_CLUSTERS = f"{global_path}/{model}/reduced_blast_princ_comp_{model}_clusters" # Clusters de la composante principale
OUTPUT_CSV = f"{global_path}/{model}_validation_score/reduced_blast_cluster_validation_scores_{model}"   # Fichier de sortie, csv avec les scores
...
OUTPUT_CLUSTERS = f"{global_path}/{model}/combined_graph_alpha_princ_comp_{model}"
prin_comp_files = [f"{OUTPUT_CLUSTERS}{a}" for a in ALPHA_LIST]

ALPHA_OUTPUT_CSV = f"{global_path}/{model}_validation_score/combined_graph_alpha_cluster_validation_scores_{model}"     # Fichier de sortie, csv avec les scores
...
def traitement_graphe(com_file, prin_comp_file, output_csv):
    for param in param_vals:
        ...
        # Chargement des résultats du modèle (Composante principale de BLAST)
        model_df = pd.read_csv(prin_comp_file + f"_{param}.tsv", sep='\t')
        ...
        df_final.to_csv(output_csv + f"_{param}.csv", index=False)
        print(f"Terminé !")
